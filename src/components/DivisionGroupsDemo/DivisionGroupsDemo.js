            ...
            {range(numOfGroups).map((groupIndex) => (
              <div key={groupIndex} className={styles.group}>
                {range(numOfItemsPerGroup).map((index) => {
                  const itemId = `${id}-item-${groupIndex * numOfItemsPerGroup + index}`;
                  return (
                    <motion.div
                      layoutId={itemId}
                      key={itemId}
        ...
        {includeRemainderArea && (
          <div className={styles.remainderArea}>
            ...
            {range(remainder).map((index) => {
              const itemId = `${id}-item-${(numOfItems - 1) - index}`;
              return (
                <motion.div layoutId={itemId} key={index} className={styles.item} />
              );
            })}
          </div>
        )}
